...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        for (int i = 0; i < piece.height(); i++) {
            for (int x = 0; x < piece.width(); x++) {
                if (piece.get(i, x) && _cells[row + i][col + x]) {
                    return false;
                }
            }
        }
    ...
    boolean placeable(Piece piece) {
        ...
        for (int i = 0; i <= _height - piece.height(); i++) {
            for (int x = 0; x <= _width - piece.width(); x++) {
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int turnScore = 0;
        turnScore += _width * nrows + _height * ncols - (nrows * ncols);
        if (ncols > 0 || nrows > 0) {
            _streakLength += 1;
        } else {
            _streakLength = 0;
        }
        turnScore += _streakLength * (_width * nrows + _height * ncols);
    ...
    boolean roundOver() {
        ...
        for (int i = 0; i < _hand.size(); i++) {
            if (!_hand.isEmpty() && _hand.get(i) != null && placeable(i)) {
                return false;
            }
        }
        return true;
    ...
    boolean get(int row, int col) {
        if (row > _height || col > _width || _cells[row][col]) {
            return true;
        }
        return false;
    }
    ...
    /** The amount of cards dealt since emptying the hand
     *  or when hand was initialized. */
